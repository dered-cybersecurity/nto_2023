...
class Fuzzer:
    ...
    def __load_wordlists(self, wordlists_dir):
        ...
        self.wordlists = {}
        for f in files:
            with open(f'{wordlists_dir}/{f}', 'r') as fr:
    ...
    def fuzz(self, endpoint, config, any_method=False, any_params=False, params_limit=0):
        ...
        if any_params:
            ...
            while params_limit - len(params) > 0:
                ...
                pname = random.choice([generate_str(), random.randint(MIN_INT, MAX_INT), random.choice(self.params), random.choice(self.params), random.choice(self.params)])
                if ptype == str:
