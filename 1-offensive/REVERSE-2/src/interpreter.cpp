#include "imports.hpp"
#include <vector>
Interpreter::Interpreter(unsigned long addr, const char* code, unsigned long len):
_addr(addr), _code(code), _code_len(len)
{
}

#define MIPS_CODE ""
#define LEN 217
#define ADDRESS 0x1000

int Interpreter::_run(std::vector<int> flag)
{
    unicorn::uc_err err;
    int ans = 0;
    unicorn::uc_engine* engine;
    int val;
    err = unicorn::uc_open(unicorn::UC_ARCH_MIPS, unicorn::UC_MODE_MIPS32, &engine);
    if(err)
    {
        throw ERROR;
    }
    err = unicorn::uc_mem_map(engine, _addr, 2 * 1024 * 1024, unicorn::UC_PROT_ALL);
    err = unicorn::uc_mem_write(engine, _addr, _code , _code_len);
    val = 0;
    err = unicorn::uc_reg_write(engine, unicorn::UC_MIPS_REG_V0, &val); 
    val = 0x2c2c2c2c;
    err = unicorn::uc_reg_write(engine, unicorn::UC_MIPS_REG_A0, &val); 
    val = flag[0];
    err = unicorn::uc_reg_write(engine, unicorn::UC_MIPS_REG_T1, &val); 
    val = flag[1];
    err = unicorn::uc_reg_write(engine, unicorn:: UC_MIPS_REG_T2, &val); 
    val = flag[2];
    err = unicorn::uc_reg_write(engine, unicorn:: UC_MIPS_REG_T3, &val); 
    val = flag[3];
    err = unicorn::uc_reg_write(engine, unicorn::UC_MIPS_REG_T4, &val); 
    val = flag[4];
    err = unicorn::uc_reg_write(engine, unicorn::UC_MIPS_REG_T5, &val); 
    val = flag[5];
    err = unicorn::uc_reg_write(engine, unicorn::UC_MIPS_REG_T6, &val); 
    val = flag[6];
    err = unicorn::uc_reg_write(engine, unicorn::UC_MIPS_REG_T7, &val); 
    err = unicorn::uc_emu_start(engine, _addr , _addr + _code_len, 0, 0);
    if(err)
    {
        std::cout << unicorn::uc_strerror(err) <<  std::endl;
        throw ERROR;
    }
    int res;
    unicorn::uc_reg_read(engine, unicorn::UC_MIPS_REG_V1, &res);
    unicorn::uc_close(engine);
    return res;
}

int Interpreter::check(std::string val)
{
    std::vector<int> inp;
    int ret = 1;
    if(val.size() != 28)
        return (ret * 0);
    for(int a = 0; a < 28; a += 4)
    {
       inp.push_back(((int)val[a] << 24) | ((int)val[a + 1] << 16) | ((int)val[a + 2] << 8) | (int)val[a+3]); 
    }
    ret = _run(inp);
    return ret;
}